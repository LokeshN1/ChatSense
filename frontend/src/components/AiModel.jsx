...
const AiModal = ({ closeModal }) => {
    ...
    // Updated getUserName function to handle name strings directly
    const getUserName = (userRef) => {
        ...
        const userId = typeof userRef === 'object' ? userRef?.id || userRef?.user_id : userRef;
        
        // Check conversation participants
        if (summary?.conversation?.participants) {
            const participant = summary.conversation.participants.find(p => p.id === userId);
            if (participant) {
                return participant.name;
            }
        }
        
        // Fallback to authUser and selectedUser
        if (userId === authUser?._id) return authUser?.fullName || "You";
        if (userId === selectedUser?._id) return selectedUser?.fullName || selectedUser?.name || "Other User";
        
        // If still no match, create a meaningful placeholder
    ...
    const formatParticipants = () => {
        // Use the conversation display_name directly
        if (summary?.conversation?.display_name) {
            return summary.conversation.display_name;
        }
        
        // Fallback: construct from participants
        if (summary?.conversation?.participants && summary.conversation.participants.length > 0) {
    ...
    // Enhanced renderList with consistent expandable functionality
    const renderList = (items, limit = 3, sectionKey = null, itemClassName = "") => {
    ...
    // Enhanced renderTopics with expandable functionality
    ...
    // Enhanced renderMainDiscussion with expandable functionality
    const renderMainDiscussion = (mainDiscussion, limit = 3) => {
        if (!mainDiscussion) {
            return <p className="text-sm text-gray-400">No key discussion points identified</p>;
        }
    ...
    // Enhanced renderDecisions with expandable functionality
            ...
            <div className="space-y-2">
                <div className="space-y-2">
                    {displayDecisions.map((decision, index) => (
                        <div key={index} className="flex items-start gap-2 p-2 bg-base-100 rounded-lg">
    ...
    const SummaryCard = ({ title, icon: Icon, children, className = "" }) => (
        <div className={`bg-base-200 rounded-xl p-4 hover:shadow-md transition-all duration-200 ${className}`}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-base-100 rounded-2xl p-6 w-full max-w-4xl max-h-[90vh] flex flex-col shadow-xl">
                ...
                <div className="tabs tabs-boxed mb-4">
                    <a className={`tab ${activeTab === 'summary' ? 'tab-active' : ''}`} onClick={() => setActiveTab('summary')}>
                        Chat Summary
                    </a>
                    <a className={`tab ${activeTab === 'query' ? 'tab-active' : ''}`} onClick={() => setActiveTab('query')}>
                        Ask a Question
                    </a>
                </div>
                                ...
                                <div className="animate-fade-in space-y-4">
                                    ...
                                    <div className="flex justify-between items-center">
                                        <div className="space-y-1">
                                            <h3 className="text-lg font-semibold text-base-content">Chat Analysis</h3>
                                            <p className="text-sm text-base-content/70">
                                               Conversation between {formatParticipants()} ({summary.analysis?.message_count || 0} messages)
                                            </p>
                                        </div>
                                        <button onClick={clearSummary} className="btn btn-xs btn-ghost">Clear</button>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <SummaryCard title="Main Discussion" icon={MessageSquare}>
                                            {renderMainDiscussion(summary.analysis?.summary?.mainDiscussion)}
                                        </SummaryCard>
                                        ...
                                        <SummaryCard title="Important Details" icon={TrendingUp}>
                                            <div className="space-y-3">
                                                {/* Dates */}
                                                {summary.analysis?.entities?.dates && summary.analysis.entities.dates.length > 0 && (
                                                    <div>
                                                        <div className="flex items-center gap-1 mb-1">
                                                            <Calendar className="w-3 h-3 text-primary" />
                                                            <span className="text-xs font-medium text-gray-600">Dates</span>
                                                        </div>
                                                        {renderList(summary.analysis.entities.dates, 3, 'dates')}
                                                    </div>
                                                )}

                                                {/* Locations */}
                                                {summary.analysis?.entities?.locations && summary.analysis.entities.locations.length > 0 && (
                                                    <div>
                                                        <div className="flex items-center gap-1 mb-1">
                                                            <MapPin className="w-3 h-3 text-primary" />
                                                            <span className="text-xs font-medium text-gray-600">Locations</span>
                                                        </div>
                                                        {renderList(summary.analysis.entities.locations.filter(loc => loc.trim() && loc !== "'s"), 3, 'locations')}
                                                    </div>
                                                )}

                                                {/* People mentioned */}
                                                {summary.analysis?.entities?.people && summary.analysis.entities.people.length > 0 && (
                                                    <div>
                                                        <div className="flex items-center gap-1 mb-1">
                                                            <Users className="w-3 h-3 text-primary" />
                                                            <span className="text-xs font-medium text-gray-600">People</span>
                                                        </div>
                                                        {renderList(summary.analysis.entities.people, 3, 'people')}
                                                    </div>
                                                )}

                                                {/* Organizations */}
                                                {summary.analysis?.entities?.organizations && summary.analysis.entities.organizations.length > 0 && (
                                                    <div>
                                                        <div className="flex items-center gap-1 mb-1">
                                                            <Briefcase className="w-3 h-3 text-primary" />
                                                            <span className="text-xs font-medium text-gray-600">Organizations</span>
                                                        </div>
                                                        {renderList(summary.analysis.entities.organizations, 3, 'organizations')}
                                                    </div>
                                                )}

                                                {/* Money */}
                                                {summary.analysis?.entities?.money && summary.analysis.entities.money.length > 0 && (
                                                    <div>
                                                        <div className="flex items-center gap-1 mb-1">
                                                            <DollarSign className="w-3 h-3 text-primary" />
                                                            <span className="text-xs font-medium text-gray-600">Money</span>
                                                        </div>
                                                        {renderList(summary.analysis.entities.money, 3, 'money')}
                                                    </div>
                                                )}
                                                ...
                                                {!summary.analysis?.entities?.dates?.length && 
                                                 ...
                                                 !summary.analysis?.entities?.organizations?.length && (
                                                    <p className="text-sm text-gray-400">No specific details mentioned</p>
                                                )}
                                            </div>
                                        </SummaryCard>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-center gap-4 p-4 bg-base-200 rounded-xl">
                                        <span className="text-sm font-medium text-base-content/70">Overall Sentiment:</span>
                                        <span className={`badge badge-lg ${
                                            (summary.analysis?.summary?.overallTone || summary.analysis?.sentiment)?.toLowerCase() === 'positive' ? 'badge-success' : 
                                            (summary.analysis?.summary?.overallTone || summary.analysis?.sentiment)?.toLowerCase() === 'negative' ? 'badge-error' : 
                                            'badge-ghost'
                                        }`}>
                                            {summary.analysis?.summary?.overallTone || summary.analysis?.sentiment || 'Neutral'}
                                        </span>
                                    </div>
                                </div>
                            )}
                    ...
                    {activeTab === 'query' && (
                        <div className="space-y-4">
                            <form onSubmit={handleQuerySubmit}>
                                ...
                                <button type="submit" className="btn btn-secondary w-full mt-2" disabled={isQuerying}>
                                    {isQuerying ? <><Loader className="animate-spin" /> Thinking...</> : "Ask AI"}
                                </button>
                            </form>
                            {isQuerying && <div className="text-center p-4"><span className="loading loading-dots loading-lg"></span></div>}
                            {queryResult && (
                                <div className="mt-4 p-4 bg-base-200 rounded-lg animate-fade-in">
                                    <div className="flex justify-between items-center mb-3">
                                        <span className="text-sm font-semibold">Q&A</span>
                                        <button onClick={clearQueryResult} className="text-error text-sm hover:underline">Clear</button>
                                    </div>
                                    
                                    {/* Question */}
                                    <div className="mb-3 p-3 bg-primary/5 rounded border-l-2 border-primary">
                                        <div className="text-xs font-extrabold text-primary mb-1">Your Query:</div>
                                        <div className="text-sm text-base-content">{queryResult.question || lastAskedQuestion || "Question not available"}</div>
                                    </div>
                                    
                                    {/* Answer */}
                                    <div className="p-3 bg-secondary/5 rounded border-l-2 border-secondary">
                                        <div className="text-xs font-extrabold text-secondary mb-1">Answer:</div>
                                        <div className="text-sm text-base-content">{queryResult.answer || queryResult.response || "Sorry, I couldn't find an answer to your question."}</div>
                                    </div>
